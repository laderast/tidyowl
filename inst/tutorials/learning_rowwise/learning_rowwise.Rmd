...
`c_across()` uses the same arguments as `across()`: `cols`, to select the columns (which can use `tidyselect` helpers, like `across()`), and optionally, `.fns`, which is the function you want to apply to all the columns.
...
Thanks to Rebecca Barter for suggesting that I flesh out the function descriptions some more. 
