...
penguins %>%
  rowwise() %>%
  summarize(species, average_bill = mean(c(bill_length_mm, bill_depth_mm)))
```
...
How do we get data out? We can use the `[[]]` (double brackets) to extract the data directly:
...
Data in a list column format can be really easily utilized by rowwise(). Here, we're counting the number of rows in each row of `data`
