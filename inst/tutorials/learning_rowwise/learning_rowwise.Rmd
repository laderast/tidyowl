...
`summarize()` and `mutate()` are very similar in a rowwise() workflow. I think of `summarize()` as having a built-in `select()`, whereas `mutate()` will return the original data plus the new columns.

In a `rowwise` workflow, you often want to apply a function to multiple columns. You can specify columns using `c()`:
...
Data in a list column format can be really easily utilized by a `rowwise()` workflow. 
...
We've only done things that you can easily do with a `group_by()/summarize()` statement so far. 
...
Note that we have to wrap the output of `lm()` with `list()`, since the returned object is a list.
